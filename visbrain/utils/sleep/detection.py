...
__all__ = ('kcdetect', 'spindlesdetect', 'remdetect', 'slowwavedetect',
           'mtdetect', 'peakdetect', 'artifacteegdetect', 'artifactecgdetect')
...
def artifacteegdetect(self, data, sf, thr, fmin, fmax):

    raw_sf = sf
    # wy 每4秒计算一次 fmin-fmax hz的功率谱
    start = []
    end = []
    powerResult = []
    peakToPeakAll = []
    gap = int(4 * raw_sf)
    num = int(len(data) / gap)
    data_raw = data
    data = filt_data(data, fmin, fmax, raw_sf, order=7)
    # 4秒间隔，每4秒求一次功率,峰峰值
    for i in range(1, len(data)-gap, gap):
        bandPwr = bandPower(data[i:i + gap], raw_sf)
        powerResult.append(bandPwr)
        # 4秒内的峰峰值
        maxI = abs(max(data_raw[i:i + gap]))
        minI = abs(min(data_raw[i:i + gap]))
        peakToPeakAll.append(maxI + minI)
    # 平均值标准差
    powerMean = np.mean(powerResult)
    powerStd = np.std(powerResult)

    # index = []
    # 阈值
    thA = powerMean + thr * powerStd
    # print("当前阈值为：%.1f，平均值为：%.4f,标准差为：%.4f,功率阈值为：%.4f" %(thr,powerMean,powerStd,thA))
    # logger.info("当前阈值为：%.1f，平均值为：%.4f,标准差为：%.4f,功率阈值为：%.4f" %(thr,powerMean,powerStd,thA))

    # thB = powerMean - thr * powerStd
    # 排序
    powerResultSorted = sorted(powerResult)
    # thA排序后所在的位置
    position = bisect.bisect(powerResultSorted,thA)
    powerResultSortedHigh = powerResultSorted[position:]
    sort = [];
    peakToPeak = [];
    powerFin = [];
    for i, v in enumerate(powerResult):

        if (v > thA):
            # index.append(i)
            start.append(i * gap + 1)
            end.append((i + 1) * gap + 1)
            # 排名、功率值、峰峰值
            sort.append(powerResultSortedHigh.index(v) + 1)
            powerFin.append(v)
            peakToPeak.append(peakToPeakAll[i])

    for i,v in enumerate(sort):
        sort[i] = len(sort) + 1 - sort[i]
    # print(sort)
    # sort.reverse()
    result = zip(start, end, sort, powerFin, peakToPeak)
    # result_show = [powerMean,powerStd,thA]
    # 将阈值相关参数显示在界面上
    self._ToolArtifactMean.setValue(powerMean)
    self._ToolArtifactStd.setValue(powerStd)
    self._ToolArtifactPowerTh.setValue(thA)
...
def bandPower(eegData, fs):
	...
	freqs, powers = signal.periodogram(eegData, fs)
	bandPwr = np.sum(powers)
	return bandPwr

###########################################################################
# Artifact ECG DETECTION
###########################################################################
# wy 第二睡眠标签
def artifactecgdetect(data, sf, hypno, rem_only, threshold, tmin=300, tmax=800,
              min_distance_ms=300, smoothing_ms=200, deriv_ms=50):
    ...
    while(k<len_wy):
        ...
        if(wy_tf[k] == True):
            ...
            while(k<len_wy):
                ...
                if(wy_tf[k] == False):
                    end_pointer = k-1;
                    end.append(end_pointer)
                    k = k+1;
                    break;
                elif(k+1 == len_wy):
                    end_pointer = len_wy-1;
                    end.append(end_pointer)
                    k=k+1;
                    break;
                else:
    ...
    # # 元组列表转列表
    # intervals = list(map(list, map(tuple.__iter__, intervals)))
    # intervals.sort()
    #
    ...
    #     if intervals[index - 1][1] >= intervals[index][0] - gap:
