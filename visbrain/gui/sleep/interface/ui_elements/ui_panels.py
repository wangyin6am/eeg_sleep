...
from visbrain.io import oversample_hypno

# import torch
# from visbrain.utils.model.engine_model import engine_model
...
class UiPanels(object):
    ...
    def __init__(self):
        ...
        # wy 迪博模型加载
        # self._PanModelEngine.clicked.connect(self._fcn_hypno_model)
    ...
    def _fcn_hypno_clean(self):
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            if self._hypno2Flag:
                self._hypno2 = np.zeros((len(self._hyp),), dtype=np.float32)
                self._hyp.set_data3(self._sf, self._hypno2, self._time)
            else:
    ...
    #         # self._hyp.clean(self._sf, self._time)
    #         # wy加载模型
    #         # file = self._file + self._ext;
    #         e = engine_model()
    #         e.loadModel()
    #         try:
    #             # e.loadData(file, None, None)
    #             e.loadDataWy(self._data, self._channels, None, self._sf)
    #         except:
    #             return "edf读取错误，检查通道名"
    #         pred = e.interface()
    #         pred = torch.argmax(pred, 1)
    #         hypno = np.array([tensor.item() for tensor in pred], dtype=np.float32)
    #
    #         self._hypno = oversample_hypno(hypno, self._N)[::self._dsf]
    #         self._hyp.set_data(self._sf, self._hypno, self._time)
    #         # Update info table :
    #         self._fcn_info_update()
    #         # Update scoring table :
