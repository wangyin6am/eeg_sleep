...
class UiSettings(object):
    ...
    def _fcn_slider_settings(self):
        ...
        win = self._SigWin.value()
        # Set minimum :
        self._SlVal.setMinimum(round(self._time.min()))
        # Set maximum :
        step = self._SigSlStep.value()
        self._SlVal.setMaximum((round((self._time.max() - win) / step) + 1))
        self._SlVal.setTickInterval(round(step))
        self._SlVal.setSingleStep(round(step))
        self._SlGoto.setMaximum(round((self._time.max() - win)))
        # Re-set slider value :
        self._SlVal.setValue(round(sl * self._SlVal.maximum() / slmax))
    ...
    def on_mouse_wheel(self, event):
        ...
        self._SlVal.setValue(round(self._SlVal.value() + event.delta[1]))
