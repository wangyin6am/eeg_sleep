...
def oversample_hypno(hypno, n):
    ...
    # wy置信度 1秒1个标签： 1 * sf = 100 如果是其他数据，把下面注释掉 用上面的
    # rep_nb = 100;
    # Repeat hypnogram :
...
def oversample_coefficient(hypno, n):
    """Oversample hypnogram.
...
# wy 读第二睡眠标签
def read_hypno2(filename, time=None, datafile=None):
    # Test if file exist :
    # assert os.path.isfile(filename), "No hypnogram2 file %s" % filename
    # Extract file extension :
    file, ext = os.path.splitext(filename)
...
# wy 读置信度
def read_coefficient(path, n, time=None, datafile=None):

    # # Test if file exist :
    assert os.path.isfile(path), "No hypnogram_coefficient file %s" % path
    #
    # # Extract file extension :
    # file, ext = os.path.splitext(filename)
    # path = os.path.splitext(filename)[0] + '_coefficient.txt'
    # Test if coefficient_file exist :

    # Load the coefficient :
    coefficient = np.genfromtxt(path, delimiter='\n', usecols=[0],
                        dtype=None, skip_header=0, encoding='utf-8')
    logger.info("coefficient successfully loaded (%s)" % path)
    coefficient = vispy_array(coefficient)
    # Get the repetition number :
    # rep_nb = int(np.round(n / len(coefficient)))
    # 重复 fs * 30秒
    # rep_nb = int(np.round(n / len(coefficient)))
    # wy置信度- 随时改
    rep_nb = 100
    # Repeat hypnogram :
    coefficient = np.repeat(coefficient, rep_nb)
    npts = len(coefficient)

    # Check size
    if npts < n:
        coefficient = np.append(coefficient, coefficient[-1] * np.ones((n - npts)))
    elif npts > n:
        coefficient = coefficient[0:n]

    return coefficient.astype(int)
